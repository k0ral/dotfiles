...
add_binds("normal", {
    ...
    key({"Control"}, "o",           function (w, c) w:enter_cmd(":open ")    end),
    buf("^t$",                      function (w, c) w:enter_cmd(":tabopen ") end),
    buf("^w$",                      function (w, c) w:enter_cmd(":winopen ") end),
    key({"Control"}, "O",           function (w, c) w:enter_cmd(":open "    .. (w.view.uri or "")) end),
    ...
    -- History
    key({"Control"}, "Left",        function (w, m) w:back(m.count)    end),
    key({"Control"}, "Right",       function (w, m) w:forward(m.count) end),
    ...
    key({"Control"}, "<",           function (w, m) w.tabs:reorder(w.view, w.tabs:current() - m.count) end, {count=1}),
    key({"Control"}, ">",           function (w, m) w.tabs:reorder(w.view, (w.tabs:current() + m.count) % w.tabs:count()) end, {count=1}),
...
--    buf("^gh$",                     function (w)       w:navigate(globals.homepage) end),
    ...
    key({},          "F5",          function (w) w:reload() end),
    key({"Shift"},   "F5",          function (w) w:reload(true) end),
    key({"Control"}, "s",           function (w) w:stop() end),

    -- Config reloading
    key({"Meta"},     "F5",          function (w) w:restart() end),
